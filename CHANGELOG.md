...
* **checkpoint 文件名由 sink 侧自守（`py/path-injection` 215/216，当前不可利用但无防线）**：`TaskCheckpoint._path()` 返回 `checkpoint_dir / f"{task_id}.json"`，其下游是 read / write / **unlink**。入站的幂等键正则 `^[A-Za-z0-9_.-]{1,64}$` 确实排除了分隔符，但 `remove_checkpoint()` 也会被历史删除路径以 DB 回读的 task_id 调用——白名单不该只长在入站那一头。现 `_path()` 直接拒绝含分隔符 / `..` / 空的 ID，并再断言 resolve 后仍落在 `checkpoint_dir` 内（`ValueError`，不静默降级）。测试：`tests/test_checkpoint_ttl.py::TestTaskIdCannotEscapeCheckpointDir` 两条。
